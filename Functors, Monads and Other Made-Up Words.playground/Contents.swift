...
//:
//: Let's write an implementation that matches this signature:
...
//: You can probably imagine a function like `dictionaryFromJSONFile` that returned a Result<Dictionary> (this type would mean that it returned a Result containing either a Dictionary or an NSError). Let's write an implementation of `map` for Result:
...
//:
//: An applicative (or applicative functor) is almost the same as a functor (in fact it's a "sub-type"), except the transform function that is passed is itself contained in a functor too. I'm not going to say much about Applicatives, but instead just show this:

func apply<I, O>(value: I?, transform: ((I) -> O)?) -> O? {
...
//: Note that this function is almost the same as our map function:
//:
//:     func map<T, U>(optional: T?, transform: (T) -> U) -> U?
//:     func apply<T, U>(value: T?,  transform: (T -> U)?) -> U?
//:
//: except that the transform function itself is inside an Optional. And that's all you need to know about Applicatives!
...
//: Explain that what we want is an implementation like map but that flatten everything at the end
...
//: Alright, easy peasy. I mentioned a `dictionaryFromJSONFile` function when I introduced the Result type earlier. Let's go through
...
//: Alright, two functions that take a plain value and return a Result with either a transformed value or an error. If you squint, these two functions actually look almost exactly the same, and are probably very similar to how we could wrap many other failable Cocoa APIs.
//:
...
let path2 = Result.Value(Box("some.json"))
let json3 = path2 >>== loadTextFile >>== parseJSON
json3.description

let path3 = Result.Value(Box("none.json"))
let json4 = path3 >>== loadTextFile >>== parseJSON
json4.description
...
//: - https://github.com/gfontenot/talks/blob/master/Functors/Functors.md
//: - http://dscoder.com/DerivingMap/
//: - http://blogs.msdn.com/b/wesdyer/archive/2007/01/18/why-functional-programming-is-important-in-a-mixed-environment.aspx
